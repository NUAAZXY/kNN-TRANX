...
def main(args):
    ...
    params, gridsearch, map_location, act_dict, grammar, primitives_type, device, is_cuda, path_folder_config, vocab = build_data()
    ...
    train_set = Dataset(
        pd.read_csv('/home/ubuntu/knnmt/knnbox/models/BertranX/dataset/conala-corpus/' + 'conala-train.csv'))
    train_epoch_itr = train_set.batch_iter(1)
    val_set = Dataset(
        pd.read_csv('/home/ubuntu/knnmt/knnbox/models/BertranX/dataset/conala-corpus/' + 'conala-val.csv'))
    val_epoch_itr = val_set.batch_iter(1)
    ...
    lr = args.lr[0]
    optimizer = AdamW(model.combiner.parameters(), lr=lr, eps=args.adam_eps, betas=(0.9, 0.98))
    ...
    while lr > args.min_lr and epoch <= max_epoch:
        ...
        for step, batch_examples in enumerate(train_set.batch_iter(1)):
            ...
            final_prob = model.get_normalized_probs(log_probs=False)
        ...
        model.eval()
        tr_loss = []
        for step, batch_examples in enumerate(val_set.batch_iter(1)):
            batch_examples = [e for e in batch_examples if len(eval(e.snippet_actions)) <= params['len_max']]
            optimizer.zero_grad()
            x, src = model(batch_examples)
            final_prob = model.get_normalized_probs(log_probs=False)
            loss = model.get_loss(final_prob, batch_examples, x, src)
            loss = -loss[0]
            loss = torch.mean(loss)
            tr_loss.append(loss)
        ...
        val_loss = sum(tr_loss)/len(tr_loss)
        if val_loss < loss_min:
            loss_min = val_loss
            patience = 0
        else:
            patience += 1
        print(epoch, val_loss, patience)
        if patience == 5:
            break
    torch.save(model.state_dict(),
               args.save_dir + 'combiner.pt'
               )
