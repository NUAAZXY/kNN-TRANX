...
def calculate_combined_prob(knn_prob, neural_model_logit, lambda_, log_probs):
    ...
    """
    # print(neural_model_logit.size())
    # neural_model_prob = F.softmax(neural_model_logit, dim=-1)
    combined_probs = knn_prob * lambda_ + neural_model_logit * (1 - lambda_)
    # print(torch.argmax(neural_model_logit, dim=-1), torch.argmax(knn_prob, dim=-1))
    # print(torch.max(neural_model_logit, dim=-1).values, torch.max(knn_prob, dim=-1).values)
    # print(torch.argmax(combined_probs, dim=-1), torch.max(combined_probs, dim=-1).values)
    # some extra infomation
    extra = {}
    extra["neural_probs"] = neural_model_logit
