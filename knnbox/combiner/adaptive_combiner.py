...
class AdaptiveCombiner(nn.Module):
    ...
    def __init__(self,
        ...
        self.trust = nn.Sequential(
             nn.Linear(probability_dim * 2 + int(math.log2(max_k)) + 1, 1),
           # nn.Tanh(),
           # nn.Dropout(p=0),
           # nn.Linear(16, 1),
             nn.Sigmoid()
    ...
    def get_knn_prob(self, vals, distances, device="cuda:0"):
        ...
        if self.k_trainable:
            ...
            #print(k_probs,knn_prob, knn_weight, vals)
    ...
    def get_combined_prob(self, knn_prob, neural_model_logit, lambda_, log_probs):
        r""" get combined probs of knn_prob and neural_model_prob """

        
        lambda_ = self.trust(torch.cat([knn_prob, neural_model_logit, self.k_probs.squeeze(0)], dim=-1))
    ...
    @classmethod
    def load(cls, path, dim, max_k):
        r""" load the adaptive knn-mt from disk """
        # config = read_config(path)
        adaptive_combiner = AdaptiveCombiner(max_k,
                                             probability_dim=dim,
                                             k_trainable=True,
                                             lambda_trainable=True,
                                             temperature_trainable=True, )

        adaptive_combiner.load_state_dict(torch.load(os.path.join(path, "adaptive_combiner.pt")), strict=False)
