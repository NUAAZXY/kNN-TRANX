    ...
    def forward(
        ...
        if self.args.knn_mode == "build_datastore":
            def get_4_gram(target):
                ...
                target = target[:, :, None]
                target_pad_1 = torch.cat(
                    (torch.zeros((batch_size, 1, 1), device=x.device, dtype=torch.long), target[:, :-1]), 1)
                target_pad_2 = torch.cat(
                    (torch.zeros((batch_size, 2, 1), device=x.device, dtype=torch.long), target[:, :-2]), 1)
                target_pad_3 = torch.cat(
                    (torch.zeros((batch_size, 3, 1), device=x.device, dtype=torch.long), target[:, :-3]), 1)
            ...
            # print('target', target, 'ids_4_gram', ids_4_gram, 'target_prob', target_prob, 'probs_4_gram', probs_4_gram, 'entropy', entropy)
            # process pad 
...
class KNNTRANX(TransformerDecoder):
    r"""
    The pck knn-mt Decoder, equipped with knn datastore, retriever and combiner.
    """
